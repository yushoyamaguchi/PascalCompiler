...
public class Compiler {
	...
	public List<String> _lineList;
	public List<String> _tokenList;
	...
	public static void main(final String[] args) {
		...
		CaslSimulator.run(TMP_OUT_CAS, TMP_OUT_ANS,"7","44");
	...
	public Node ifThenStatement(int line) throws EnshudSyntaxErrorException {
		...
		node.children.add(conditionalStatement(node.getLine()));
		node.setLine(node.children.get(0).getLine());
		node=wordCheckAndLine(node,"STHEN");
		node.children.add(thenStatement(node.getLine()));
		node.setLine(node.children.get(1).getLine());
		if(wordCheck(node.getLine(),"SELSE")) {
			node=wordCheckAndLine(node,"SELSE");
			node.children.add(elseStatement(node.getLine()));
			node.setLine(node.children.get(2).getLine());
		}
		return node;
	}

	public Node thenStatement(int line) throws EnshudSyntaxErrorException {
		Node node=createNode(line,"ThenStatement");
		node.children.add(compoundStatements(node.getLine()));
		node.setLine(node.children.get(0).getLine());
		return node;
	}

	public Node elseStatement(int line) throws EnshudSyntaxErrorException {
		Node node=createNode(line,"ElseStatement");
		node.children.add(compoundStatements(node.getLine()));
		node.setLine(node.children.get(0).getLine());
		return node;
	}
	...
	public Node conditionalStatement(int line) throws EnshudSyntaxErrorException {
		Node node=createNode(line,"ConditionalStatement");
	...
	public Node procedureStatement(int line) throws EnshudSyntaxErrorException {
		...
		node.setLine(node.children.get(0).getLine());
		node.children.add(parameterUse(node.getLine()));
		node.setLine(node.children.get(1).getLine());
		return node;
	}

	public Node parameterUse(int line) throws EnshudSyntaxErrorException  {
		Node node=createNode(line,"ParameterUse");
		if(wordCheck(node.getLine(),"SLPAREN")) {
			node=wordCheckAndLine(node,"SLPAREN");
			node.children.add(formulaList(node.getLine()));
			node.setLine(node.children.get(0).getLine());
	...
	public Node variableRead(int line) throws EnshudSyntaxErrorException {
		...
		if(wordCheck(node.getLine(),"SLBRACKET")) {
			...
			parentNode.children.add(subscriptRead(parentNode.getLine()));
			parentNode.setLine(parentNode.children.get(1).getLine());
	...
	public Node subscriptRead(int line) throws EnshudSyntaxErrorException {
		Node node=createNode(line,"Subscript");
		node.children.add(formulaRead(node.getLine()));
		node.setLine(node.children.get(0).getLine());
		return node;
	}
	...
	public void libraryOutput (final String outputFileName) {
		try {
			final List<String> buffer = Files.readAllLines(Paths.get("data/cas/lib.cas"));
			FileWriter fw = null;
       	    fw=new FileWriter(outputFileName,true);
            PrintWriter pw = new PrintWriter(fw);
	...
	public void caslGenerate(final String outputFileName) {
		   try {
			    fileClear(outputFileName);
			    caslHeadOutput(outputFileName);
				VisitorInfo vi=_root.accept(new CompilerVisitor(this));
				caslContentsOutput(vi,outputFileName);
				caslEndOutput(vi,outputFileName);
				libraryOutput(outputFileName);
	...
	public void caslHeadOutput (final String outputFileName) {
		try{
			FileWriter fw = null;
       	    fw=new FileWriter(outputFileName,true);
            PrintWriter pw = new PrintWriter(fw);
	...
	public void caslContentsOutput (VisitorInfo vi,final String outputFileName) {
		try{
			FileWriter fw = null;
       	    fw=new FileWriter(outputFileName,true);
            PrintWriter pw = new PrintWriter(fw);
            for(String casl:vi.cv.getCaslLines()) {
            	pw.println(casl);
            }
            for(SubCompVisitor scv :vi.cv.getSubCompList()) {
            	subProOutput(scv,fw,pw);
            }
            pw.close();
            fw.close();
	...
	public void subProOutput(SubCompVisitor scv,FileWriter fw,PrintWriter pw) {
        for(String casl:scv.getCaslLines()) {
        	pw.println(casl);
        }
	}

	public void caslEndOutput (VisitorInfo vi,final String outputFileName) {
		try{
			FileWriter fw = null;
       	    fw=new FileWriter(outputFileName,true);
            PrintWriter pw = new PrintWriter(fw);
            pw.println("VAR\tDS\t"+vi.getNumOfVar());
            pw.println("READ\tDS\t1");
            stringOutput(vi,pw,fw);
	...
	public void stringOutput(VisitorInfo vi,PrintWriter pw,FileWriter fw) {
        for(String casl:vi.cv.getStringList()) {
        	pw.println(casl);
        }
	}

	public void fileClear(final String outputFileName) {
		try {
			FileWriter fw = null;
